...
proctype high()	/* can run at any time */
{
end:	do
		:: atomic
		   {
		   		!(h_state == idle && l_state == running && mutex == busy) &&
		   		!(h_state == idle && l_state == waiting && mutex == busy)
		   		h_state = waiting;
		   }
			atomic 
			{ 
...
proctype low() provided (h_state == idle) /* scheduling rule */
{
end:	do
		:: l_state = waiting;
			atomic 
